...
def run_tracker(model_path_players: str,model_path_ball: str, video_path: str,trackerP:str,trackerB:str):
    ...
    tracker_name = os.path.splitext(os.path.basename(trackerP))[0]

    output_dir = os.path.join("outputs", f"{video_name}_{model_name}_{tracker_name}_classid")
    ...
    while cap.isOpened():
        ...
        results = model_player.track(frame,verbose=False,tracker=trackerP)[0]
        results_ball = model_ball.track(frame,verbose=False,tracker=trackerB)[0]
        detections = []
        best_by_class = {}
...
if __name__ == "__main__":
    ...
    run_tracker(args.modelP,args.modelB, args.video,args.trackerP,args.trackerB)
