...
class PlayerTracker:
    ...
    def process_single_frame(self, frame_idx,trackers):
        ...
        for i,det in enumerate(detections):
            if det['class_id'] not in current_ids:            
                ...
                tracker:cv2.legacy.TrackerCSRT = self.tracker()
        ...
        for v in trackers:
            tracker:cv2.legacy.TrackerCSRT = v['tracker']
    ...
    def process_frames(self):
        ...
        frame = cv2.imread(self.image_paths[start_idx])
        trackers = []
        ...
        for frame_idx in tqdm(range(len(self.image_paths))):
            predicted_boxes,trackers = self.process_single_frame(frame_idx,trackers)
